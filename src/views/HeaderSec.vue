<template>
  <div class="div">
      <ul>
        ...
        <li>
            <input type="text" v-model="movieName" @keydown.enter="GoToSearch">
            <div class="bar" v-show="movieName">
        ...
        <li @click="GoToPerson">
            <img :src="$store.state.adv ? $store.state.adv : 'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png'" :title="$store.state.admin">
        </li>
      </ul>
  </div>
</template>
...
import { warning } from '@/utils/popBox'
export default {
    ...
    setup( ) {
        ...
        function send( ){
            axios({
                url:"http://localhost:8088/api/searchMovie",
                method:"GET",
                params:{ 
                ...
                dataList.push(...res.data.result)
            } )
        }
        
        watch( ( ) => movieName.value ,( newValue ) =>{
            if( newValue.trim( ) == "") {
                dataList.splice(0,dataList.length)
                return
            }else{
...
.bar{ 
    ...
    width: 300px;
    height: auto;
    box-shadow: 0 0 5px rgb(90, 89, 89);
